...
test('newsletter combines feature guidance, hides empty sections, and explains popularity without public counts', async () => {
  ...
  assert.match(page, /\{newFeaturesSection\}/);
  assert.ok(page.indexOf("conciseEditorial&&visible('feature_details')") < page.indexOf('{testimonialsSection}'));
  ...
  assert.ok(page.indexOf('{newFeaturesSection}') < page.indexOf("visible('parent_tips')"));
  assert.ok(page.indexOf('{newFeaturesSection}') < page.indexOf("visible('membership_details')"));
  ...
  assert.match(server, /parent_tips: tips/);
  ...
  assert.match(server, /title: normalizeUpcomingNewsletterTitle\(savedDraft\.title\)/);

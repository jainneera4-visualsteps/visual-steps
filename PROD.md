...
Apply `database_updates/2026-09-02_support_inbox.sql` before deploying the Support Inbox routes. Confirm that `SUPABASE_SERVICE_ROLE_KEY` is configured for server-side message storage and administrator access, and that SMTP plus `CONTACT_TO_EMAIL` are configured for Contact notifications and in-app replies. After deployment, submit one Contact-page message, verify it appears under **Admin → Support Inbox**, send a test reply, and confirm the conversation becomes resolved.

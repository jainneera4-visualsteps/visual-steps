...
import { ExternalLink, Mail, Send, ShieldCheck, X } from 'lucide-react';
...
export function IssueCard({issue}:{issue:Issue}){
  ...
  return <article className="newsletter-copy surface p-6 sm:p-8"><p className="text-xs font-black uppercase text-brand-700">{displayDate(issue.issue_date)}</p><h3 className="mt-2 text-2xl font-black">{issue.title}</h3><p className="mt-3 text-slate-600">{issue.introduction}</p><div className="mt-6 grid gap-5 lg:grid-cols-2">
    {visible('feature_previews')&&<Section fullWidth tone="violet" title={title('feature_previews','Feature Previews')} items={(issue.feature_previews||[]).map((x:any)=>{
    ...
    })}/>} {visible('community_posts')&&<Section tone="amber" title={title('community_posts','Parent Stories, News, Information, Tips and Tricks')} items={(issue.community_posts||[]).map((x:any)=><><b>{x.title}</b> ({x.type}) — {x.content} — {x.displayName}{x.sourceUrl&&<a href={x.sourceUrl} target="_blank" rel="noreferrer" className="ml-1 underline">Source</a>}{x.editorialContext&&<small className="mt-2 block italic">{x.editorialContext}</small>}</>)}/>} {visible('parent_testimonials')&&<Section tone="rose" title={title('parent_testimonials','Parent Testimonials')} items={(issue.parent_testimonials||[]).map((x:any)=><>“{x.quote}” — <b>{x.displayName}</b>{x.editorialContext&&<small className="mt-2 block italic">{x.editorialContext}</small>}</>)}/>}
  ...
  </div><NewsletterLinks links={issue.footer_links}/></article>;
}

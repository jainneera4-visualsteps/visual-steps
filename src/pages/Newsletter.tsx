...
function FeatureNavigationSteps({steps}:{steps:string[]}){
  if(!steps.length)return null;
  return <div className="newsletter-feature-navigation mt-4"><div className="newsletter-feature-navigation-lead"><b>Where to find it</b><ol className="mt-2 list-decimal space-y-2 pl-5"><li>{steps[0]}</li></ol></div>{steps.length>1&&<ol start={2} className="mt-2 list-decimal space-y-2 pl-5">{steps.slice(1).map((step,index)=><li key={index}>{step}</li>)}</ol>}</div>;
}
...
export function IssueCard({issue}:{issue:Issue}){
  ...
  return <NewsletterFlipBook issueTitle={issue.title}><header className="newsletter-page"><p className="text-sm font-black uppercase tracking-wider text-brand-700">{displayDate(issue.issue_date)}</p><h3 className="mt-3 text-4xl font-black sm:text-5xl">{issue.title}</h3><p className="mt-5 text-base leading-8 text-slate-600">{issue.introduction}</p><NewsletterLinks links={issue.footer_links}/></header><div className="grid gap-7">
    {visible('feature_previews')&&<Section oneItemPerPage fullWidth tone="violet" title={title('feature_previews','Feature Previews')} items={(issue.feature_previews||[]).map((x:any)=>{
      ...
      const featureId=featureIdFor(x);
      return <>{x.compact?<div className="newsletter-feature-heading"><span className="mb-2 inline-flex rounded-full bg-blue-100 px-2.5 py-1 text-xs font-black uppercase tracking-wider text-blue-800">{x.changeType==='updated'?'Feature update':'New feature'}</span><b className="block text-slate-900">{x.title}</b></div>:<><span className="mb-2 inline-flex rounded-full bg-blue-100 px-2.5 py-1 text-xs font-black uppercase tracking-wider text-blue-800">{x.changeType==='updated'?'Feature update':'New feature'}</span><b className="block text-slate-900">{x.title}</b></>}{x.compact&&Array.isArray(x.bullets)?<><ul className="mt-3 list-disc space-y-2 pl-5">{x.bullets.map((bullet:any,index:number)=><li key={index}><b>{bullet.label}:</b> {bullet.text}</li>)}</ul>{Array.isArray(x.navigationSteps)&&<FeatureNavigationSteps steps={x.navigationSteps}/>}</>:<><span className="mt-1 block">{x.summary}</span><span className="mt-2 block">{x.details}</span>{x.familyImpact&&<span className="mt-2 block"><b>How this supports growth:</b> {x.familyImpact}</span>}<small className="mt-2 block font-semibold text-brand-800">Where to find it: {x.help}</small></>}{featureId&&<Link to={`/features/${featureId}`} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex font-bold text-brand-800 underline underline-offset-4">Read more</Link>}</>;
    })}/>} {issue.section_visibility?.concise_editorial===true&&visible('feature_details')&&<Section bulleted tone="violet" title={title('feature_details','Using Visual Steps Meaningfully')} items={issue.feature_details||[]}/>} {/* Feature preview page ends here. */}

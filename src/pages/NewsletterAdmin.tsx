...
export default function NewsletterAdmin() {
  ...
  return <div className="page-shell"><div className="page-container space-y-8">
    ...
    <section className="surface p-6"><div className="flex flex-wrap items-end gap-4"><div className="min-w-56 flex-1"><Select label="Weekly delivery day" value={deliveryWeekday} onChange={event=>setDeliveryWeekday(Number(event.target.value))}>{weekdayNames.map((day,index)=><option key={day} value={index}>{day}</option>)}</Select></div><Button onClick={saveDeliveryDay} isLoading={busy}>Save delivery day</Button></div><p className="mt-3 text-xs leading-5 text-slate-500">Subscribers will receive the newsletter on the selected weekday. You can change the delivery day here whenever your publishing schedule changes.</p></section>

...
const sectionKeys = ['feature_previews','new_features','community_posts','parent_testimonials','popular_features','recommended_resources','parent_tips','membership_details'] as const;
const weekdayNames = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
const emailItemText = (key: typeof sectionKeys[number], item: any) => {
  if (typeof item === 'string') return item;
  if (key === 'new_features') return `${item.title} — ${item.summary} ${item.details} Where to find it: ${item.help}`;
  if (key === 'feature_previews') return `${item.title} — ${item.caption}`;
  if (key === 'community_posts') return `${item.title} (${item.type}) — ${item.content} — ${item.displayName}`;
  if (key === 'parent_testimonials') return `“${item.quote}” — ${item.displayName}`;
...
export default function NewsletterAdmin() {
  ...
  const saveDeliveryDay = async () => {
    ...
    try {
      ...
      setMessage(`Newsletter delivery changed to ${weekdayNames[deliveryWeekday]} at 13:00 UTC.`);
      await load(false);

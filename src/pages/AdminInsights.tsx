...
type AiUsage = {
  ...
  features: { name: string; parentId: string | null; parentName: string; requests: number; inputTokens: number; outputTokens: number; estimatedCostUsd: number; lastUsedAt: string }[];
...
function AiUsageInterpretation({ usage }: { usage: AiUsage }) {
  ...
  const highestCost = [...usage.features].sort((a, b) => b.estimatedCostUsd - a.estimatedCostUsd)[0];
...
function waitingSince(item: RegistrationRow) {
  if (item.confirmedAt) return '—';
  const since = new Date(item.confirmationSentAt || item.createdAt).getTime();
...
export default function AdminInsights() {
  ...
  return <div className="page-shell"><div className="page-container space-y-6">
    ...
    {tab === 'overview' ? <>
      ...
      <section className="surface p-5"><div className="flex items-center gap-2"><h2 className="text-xl font-black">Registration status</h2><HelpTip text="These statuses come from Supabase authentication. They do not guarantee inbox delivery; use the configured email provider’s delivery log for delivered, bounced, or rejected messages."/></div><p className="mt-2 max-w-3xl text-sm text-slate-600">Understand when signup began, how long confirmation has been pending, whether a parent profile exists, and whether they later signed in. Delivery, bounce, and message-open results are not available from authentication data; check your email provider before following up. No child / adult information is included.</p></section>

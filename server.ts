...
const productFeatureRegistry = [
    ...
    "updates": [
      {
        ...
        "summary": "Add Activity suggests only names previously used in the selected category.",
        "details": "After a parent chooses or types an activity category, the Activity Name suggestions show names from that category rather than every saved activity name. Parents can still type a new name. Changing the category clears the previous name so an unrelated suggestion is not carried forward. If the same learner already has that category and name on the selected date, a non-blocking notice points this out before saving; intentional separate assignments remain possible. Existing activity names remain unchanged while editing.",
        "familyImpact": "Parents find relevant familiar activity names more quickly without losing the option to create a new choice for the learner.",
        "guideParagraphs": [
          "Choose the activity category first, then select a suggested activity name or enter a new one. Suggestions are based on activities already used in that category. If an assignment with the same category and name already exists for this learner on the selected date, review the notice and decide whether a separate assignment is intentional."
        ],
        "help": "In Add Activity, select Activity Category before Activity Name. The name suggestions now match the chosen category, and a new name can still be typed."
      },
      {
        "updatedOn": "2026-09-24",
        "title": "Clearer learner activity details",
        "summary": "Learners see one readable step sequence, nearby-help guidance, and a calm finish action after the steps.",
        "details": "The learner activity view displays visual steps in a single column with larger text and touch targets. Theme colors accent the steps without reducing text contrast. A compact help prompt preserves the learner's chosen spoken, sign, or card method; Message Parent returns to the existing family chat. The finish action follows the steps, while a reminder explains that the learner may take a break and return to saved step checks. Parent previews retain their existing layout.",
        "familyImpact": "A predictable reading order makes the selected activity easier to follow without turning the available choices into a checklist. Learners can ask for nearby help or send a message, pause safely, and finish at their own pace.",
        "guideParagraphs": [
          "After choosing an activity, read its steps from top to bottom. Checkmarks can mark steps that were done, and they stay saved if the learner takes a break.",
          "Use the family's familiar help phrase, sign, or card when a nearby adult can help. Message Parent opens the existing chat when a written message is more useful. Finishing the activity does not require every step to be checked."
        ],
        "help": "Open an available choice on the learner dashboard. Follow the single-column steps, use Need help or Message Parent if useful, and choose I’m finished with this activity when done. Back to List allows a break without clearing checked steps."
      },
      {
        ...
        "details": "At the learner's day-end, available activities that were not selected become Not Chosen. If the app was not open then, older choices are closed on the next visit. The parent sees each activity's status and reason, and can view, edit, reassign, or delete it in a paginated grid after Completed. The Not Chosen tab appears only when the selected learner has an item there. An activity paused, cancelled, or replaced by the parent keeps that reason. A repeating activity may create a separate future occurrence; its old occurrence is not moved. The learner does not see Not Chosen as a failure, and no tokens are deducted.",
      ...
      },
      {
        ...
        "details": "Each learner theme uses a softly tinted background and keeps essential information on reliable high-contrast surfaces. A matching companion can invite the learner to choose an activity, restrained decorations stay around the edges, and activity cards receive theme accents. Completing an activity shows a calm acknowledgement and makes clear that the learner may choose another activity, take a break, or leave. The Rewards view shows only active items the learner can currently afford; unaffordable items are not shown as locked goals or progress bars. Parents can choose a friendly character and message, a quieter theme icon, or no companion for an older learner or adult.",
        ...
        "guideParagraphs": [
          ...
          "In Rewards, only active items the learner can afford are shown, with a simple cost. If none are affordable, the learner can come back later. Theme decoration remains gentle, and device reduced-motion preferences are respected."
        ],
  ...
  },
  {
    ...
    "screenshot": {
      ...
      "alt": "Visual Steps positive recognition and bonus tokens workspace",
      "caption": "Positive recognition stands apart from the parent-only option to add bonus tokens."
    },
    "introducedOn": "2026-08-20",
    "plan": "starter",
    "icon": "gift",
  ...
  },
  {
    ...
    "summary": "Watch a focused narrated presentation, try the real Guest Login workflow, and learn how to use Visual Steps as an app.",
    "details": "The Home page presents one narrated Visual Steps tour beside the first-visit login choices, using seven real screens from the current Guest Login journey. Saved Leda narration guides visitors through one suggested activity, clear steps within that choice, the learner view, a parent-managed reward catalog, token-free positive recognition, the Use as an app guide, and optional signup. Continue as Guest opens the same realistic temporary parent workspace. Guest changes use the existing temporary session and reset when the visitor exits or reloads.",
    ...
    "updates": [
      ...
      {
        "updatedOn": "2026-09-24",
        "title": "A choice-first introduction with current real screens",
        "summary": "The narrated guest video and Quick Start show choice-first activities, token-free recognition, and a simple path to use Visual Steps as an app.",
        "details": "The seven video images show the current Guest Login workflow, including a real capture of its Use as an app dialog. Saved Leda narration explains the visible controls in a warm, unhurried style and reflects the learner's three-column dashboard, single-column activity steps, parent reward form, separate Positive Recognition view, and installation guidance. Guest Quick Start explains that an activity is an editable choice, not another list to finish. Installing does not preserve temporary guest changes; signup does. The learner Rewards view shows only active items the learner can currently afford.",
        "familyImpact": "A new parent can see the actual product and understand its choice-first philosophy before signing up, without learning an outdated reward or recognition workflow.",
        "guideParagraphs": [
          "Play the video when you want a short introduction, then choose Continue as Guest to try the same real parent and learner screens. The voice is saved; playback does not generate fresh AI narration.",
          "Use the guest Quick Start to shape one activity. The learner may choose an available activity in any order, follow its steps, ask for help, or pause. Recognition is a separate message without tokens, and rewards remain optional motivators. Select Use as an app to see the browser-specific installation steps; sign up separately if you want to keep guest work."
        ],
        "help": "On Home, select Play on the Visual Steps video, or select Continue as Guest. In guest Quick Start, choose a situation, open the suggested activity form, and preview the real learner view. Select Use as an app for installation guidance. Guest changes reset on reload; sign up to keep work."
      }
    ],
    "plan": "starter",
    "icon": "guest",
  ...
  },
  {
    ...
    "summary": "Create personalized resources, notice useful patterns, and keep rewards secondary to learning and participation.",
      ...
      html: `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid #f0f0f0; border-radius: 12px; color: #333; line-height: 1.6;">
          ...
          <div style="background-color: #eff6ff; padding: 18px 20px; border-left: 4px solid #2563eb; border-radius: 6px; margin: 25px 0;">
            ...
            <p style="margin: 6px 0 0; color: #475569;">Start with one meaningful activity. Offer it as a choice and divide it into simple steps. The learner may choose it when it works for them; there is no list they must finish.</p>
          </div>
          ...
          <p>You know the child / adult you support best. Visual Steps helps you offer meaningful choices and clear steps, while respecting their pace and agency.</p>
...
app.get('/api/activity-types', authenticateToken, async (req: any, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('activities')
      .select('activity_type, category, kids!inner(user_id)')
      .eq('kids.user_id', userId);

    if (error) throw error;

    const types = Array.from(new Set(data.map((row: any) => row.activity_type))).sort();
    const typeCategories = Array.from(new Map(data
      .filter((row: any) => String(row.activity_type || '').trim() && String(row.category || '').trim())
      .map((row: any) => [`${String(row.category).trim().toLocaleLowerCase()}\u0000${String(row.activity_type).trim().toLocaleLowerCase()}`, {
        name: String(row.activity_type).trim(), category: String(row.category).trim(),
      }])).values());
    res.json({ types, typeCategories });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
  }
...
SAFETY AND ACCURACY:
- Visual Steps is not a to-do list. Activities are parent-offered choices, not a daily quota. Preserve Activity → Steps: the chosen activity may have sequential steps, but there is no mandatory next activity and no pressure to finish all visible choices.
- Treat unchosen activities as parent planning information, not learner failure. Explain changed plans simply and offer a parent-selected alternative as a choice, never as a demand.
- Keep positive recognition separate from messages and token changes. Rewards are secondary: never recommend locked reward goals, progress bars, streaks, tasks remaining, or rushing through activities to earn tokens.

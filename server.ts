...
const productFeatureRegistry = [
  ...
  },
  {
    ...
    "details": "Visitors can first watch a short guided experience on Home using the current application screens from Start Tour and Guest Login. The demonstration can be paused, replayed, viewed by chapter, or expanded to full screen before the visitor continues into the temporary workspace. Continue as Guest then opens a realistic parent workspace where sequential onboarding bubbles point directly to menus, forms, and primary buttons while explaining the dashboard, child profile, illustrated activities, messages, verification, quiz, worksheet, social story, reports, and learner view. Guest messages, verification choices, and other changes are temporary and disappear when the page is refreshed. Content-creation tools are unavailable during guest exploration, helping visitors learn the app without using a family's daily allowance.",
    ...
    "guideParagraphs": [
      "The Home page begins with a concise visual walkthrough built from the current Start Tour and Guest Login screens. Visitors can pause on a useful screen, select a chapter, replay the presentation, use full-screen viewing, or move directly into Guest Login. The demonstration introduces the overall journey without asking the visitor to enter personal information.",
    ...
    "help": "On Home, open See the real experience to play, pause, replay, choose a chapter, or view the demonstration full screen. Select Try Guest Login or Continue as Guest to enter the interactive workspace. Follow the product-hint bubbles as they point to each menu and button, or close and replay them later. All guest changes stay in memory and reset when the page reloads.",
    "screenshot": {
